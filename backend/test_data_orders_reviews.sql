...
BEGIN
    ...
    -- Получаем ID некоторых товаров для заказов
    SELECT id INTO product1_id FROM products WHERE seller_id = seller1_id AND title LIKE 'Мужская футболка%' LIMIT 1;
    SELECT id INTO product2_id FROM products WHERE seller_id = seller2_id AND title LIKE 'iPhone%' LIMIT 1;
    SELECT id INTO product3_id FROM products WHERE seller_id = seller4_id AND title LIKE 'Кроссовки%' LIMIT 1;
    SELECT id INTO product4_id FROM products WHERE seller_id = seller5_id AND title LIKE 'Тональный%' LIMIT 1;
    SELECT id INTO product5_id FROM products WHERE seller_id = seller6_id AND title LIKE 'Конструктор%' LIMIT 1;

    -- Заказы и отзывы
    -- Заказ 1: buyer1 покупает футболку у seller1
    IF seller1_id IS NOT NULL AND product1_id IS NOT NULL THEN
        ...
        VALUES (
            ...
            seller1_id,
            jsonb_build_array(
                jsonb_build_object('product_id', product1_id, 'quantity', 2, 'price', 1200, 'discount_price', 999)
            ),
            1998.00,
    ...
    -- Заказ 2: buyer1 покупает iPhone у seller2
    IF seller2_id IS NOT NULL AND product2_id IS NOT NULL THEN
        INSERT INTO orders (id, order_number, buyer_id, seller_id, items, total_amount, delivery_address, phone_number, payment_method, status, created_at, updated_at)
        VALUES (
            order2_id,
            ...
            seller2_id,
            jsonb_build_array(
                jsonb_build_object('product_id', product2_id, 'quantity', 1, 'price', 85000, 'discount_price', 82000)
            ),
            82000.00,
            'г. Бишкек, ул. Киевская 45, кв. 12',
            '+996700111111',
    ...
    -- Заказ 3: buyer2 покупает кроссовки у seller4
    IF seller4_id IS NOT NULL AND product3_id IS NOT NULL THEN
        INSERT INTO orders (id, order_number, buyer_id, seller_id, items, total_amount, delivery_address, phone_number, payment_method, status, created_at, updated_at)
        VALUES (
            order3_id,
            ...
            seller4_id,
            jsonb_build_array(
                jsonb_build_object('product_id', product3_id, 'quantity', 1, 'price', 6500, 'discount_price', 5999)
            ),
            5999.00,
    ...
    -- Заказ 4: buyer3 покупает косметику у seller5
    IF seller5_id IS NOT NULL AND product4_id IS NOT NULL THEN
        INSERT INTO orders (id, order_number, buyer_id, seller_id, items, total_amount, delivery_address, phone_number, payment_method, status, created_at, updated_at)
        VALUES (
            order4_id,
            ...
            seller5_id,
            jsonb_build_array(
                jsonb_build_object('product_id', product4_id, 'quantity', 1, 'price', 3500, 'discount_price', 3199)
            ),
            3199.00,
    ...
    -- Заказ 5: buyer4 покупает конструктор у seller6
    IF seller6_id IS NOT NULL AND product5_id IS NOT NULL THEN
        INSERT INTO orders (id, order_number, buyer_id, seller_id, items, total_amount, delivery_address, phone_number, payment_method, status, created_at, updated_at)
        VALUES (
            order5_id,
            ...
            seller6_id,
            jsonb_build_array(
                jsonb_build_object('product_id', product5_id, 'quantity', 1, 'price', 3500, 'discount_price', 2999)
            ),
            2999.00,
    ...
    -- Дополнительные заказы без отзывов (для разнообразия)
    ...
    -- Заказ 9: buyer8 покупает у seller8 (Ош)
    ...
    -- Заказ 10: buyer2 покупает у seller2 снова
    ...
    -- Заказ 11: buyer3 покупает у seller9 (Ош)
    ...
    -- Заказ 13: buyer5 покупает у seller10 (Джалал-Абад)
    ...
    -- Несколько заказов в процессе (без отзывов)
    IF seller1_id IS NOT NULL THEN
        INSERT INTO orders (id, order_number, buyer_id, seller_id, items, total_amount, delivery_address, phone_number, payment_method, status, created_at, updated_at)
        VALUES (
            order16_id,
            'ORD-20250217-I1J2K3L4',
            buyer8_id,
            seller1_id,
            '[]'::jsonb,
            1999.00,
            'г. Бишкек, мкр. Аламедин-1',
            '+996700888888',
            'wallet',
            'processing',
            NOW() - INTERVAL '6 hours',
    ...
    IF seller8_id IS NOT NULL THEN
        ...
        VALUES (
            order20_id,
            'ORD-20250218-Y7Z8A9B0',
            buyer5_id,
            seller8_id,
            '[]'::jsonb,
            2999.00,
            'г. Ош, ул. Курманжан Датка 78',
            '+996700555555',
            'mbank',
            'pending',
