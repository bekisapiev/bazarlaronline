...
async def get_sellers_catalog(
    ...
    if tariff:
        # Filter by tariff - support comma-separated values like "pro,business"
        tariffs = [t.strip().lower() for t in tariff.split(',')]
        query = query.where(User.tariff.in_(tariffs))
    ...
    count_query = select(func.count()).select_from(SellerProfile).join(User, SellerProfile.user_id == User.id)
    if city_id:
    ...
    if tariff:
        tariffs = [t.strip().lower() for t in tariff.split(',')]
        count_query = count_query.where(User.tariff.in_(tariffs))
    if search:
        ...
        "items": [
            {
                ...
                "shop_name": row[0].shop_name,
                "description": row[0].description,
                "logo_url": row[0].logo_url,
                "banner_url": row[0].banner_url,
                "city_id": row[0].city_id,
                "seller_type": row[0].seller_type,
                "category_id": row[0].category_id,
                "rating": float(row[0].rating),
                "reviews_count": row[0].reviews_count,
                "is_verified": row[0].is_verified,
                "tariff": row[1].tariff if row[1] else None
            }
            for row in rows
        ],
        "total": total,
